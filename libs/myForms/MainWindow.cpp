...
unsigned char* MainWindow::getData(int** img)
{
...
unsigned char* MainWindow::getAlpha(int** img)
{
  unsigned char* d = (unsigned char*)malloc(w*h);
...
void MainWindow::setImg(unsigned char* data, unsigned char* alpha)
{
  for (int y = 0; y < h; y++)
  {
    for (int x = 0; x < w; x++)
    {
      int index = 3 * ((y * w) + x);
      int pix = (data[index+0] << 24) | (data[index+1] << 16) | (data[index+2] << 8) | 0x00;
      if (alpha != nullptr)
      {
        pix = pix | (alpha[index/3] & 0xFF);
      } else {
        pix = pix | 0xFF;
      }
      img[y][x] = pix;
...
{
  wxImage wxmimg = wxImage(w, h, getData(img), getAlpha(img));
